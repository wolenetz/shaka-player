...
shaka.media.StreamingEngine.prototype.initSourceBuffer_ = function(
  ...
  // Calculate the changeType's call's parameter now, since mediaState's stream
  // choice can change while we fetch the init segment.
  let fullMimeType = shaka.util.MimeUtils.getFullType(
  ...
  let appendInit = fetchInit.then(function(initSegment) {
    ...
    // HACK (this is overkill since not all init segments need to be prefixed by
    // a changeType() operation, if there is no codec or container switch
    // involved.): Here, we assume that codec switching is supported and an init
    // segment is needed, so we explicitly issue a changeType() to ensure the
    // MSE implementation is prepared for a potential bytestream or codec
    // change.
    shaka.log.v1(logPrefix, 'changeType(' + fullMimeType +
          '), and appending init segment');
    return Promise.all([
        this.playerInterface_.mediaSourceEngine.changeType(
...
shaka.media.StreamingEngine.logPrefix_ = function(mediaState) {
  return '(' + mediaState.type + ':' + mediaState.stream.id +
      ' (originalId ' + mediaState.stream.originalId + ') : ' +
      shaka.util.MimeUtils.getFullType(mediaState.stream.mimeType,
